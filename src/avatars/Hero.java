...
import items.HeroProjectile;
import items.SpecialBubble;

/**
 * A Hero is an Avatar that is controllable by the player. Of all the Avatars, only
 * Hero has KeyBindings. Hero can shoot HeroProjectiles, shield from attacks, trigger
 * a special attack and collect Fruits for points.
 * @author blancont
 *
 */
public class Hero extends Avatar {
	...
	public void shootProjectile() {
		...
		world.addProjectile(new HeroProjectile(world, x, y, direction));
	}
	...
	private void addKeyBindings(InputMap inputMap, ActionMap actionMap) {
		...
		actionMap.put("SpecialMove", new AbstractAction() {
			public void actionPerformed(ActionEvent arg0) {
				if (readyToCharge) {
					world.addSpecialBubble(new SpecialBubble(world, x, y));
